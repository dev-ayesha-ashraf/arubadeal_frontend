...
export const GlobalListingsFilter = ({ dropdowns, filters, setFilters, onApply }: Props) => {
                ...
                <div className="flex flex-col space-y-4 max-h-[calc(90vh-120px)] overflow-y-auto pl-3 pr-3">
                    ...
                    <Select value={filters.make} onValueChange={v => setFilters({ ...filters, make: v })}>
                        ...
                        <SelectContent className="max-h-60 overflow-y-auto bg-white">
                            {dropdowns.makes.map(make => <SelectItem key={make.id} value={make.name || ""}>{make.name}</SelectItem>)}
                        </SelectContent>
                    </Select>
                    ...
                    {/* Type - Using name instead of id */}
                    <Select value={filters.type} onValueChange={v => setFilters({ ...filters, type: v })}>
                        <SelectTrigger className="w-full"><SelectValue placeholder="Select Type" /></SelectTrigger>
                        <SelectContent className="max-h-60 overflow-y-auto bg-white">
                            {dropdowns.types.map(type => <SelectItem key={type.id} value={type.name || ""}>{type.name}</SelectItem>)}
                        </SelectContent>
                    </Select>

                    {/* Fuel Type - Using name instead of id */}
                    ...
                    {/* Badge filter is intentionally removed for global listings */}
